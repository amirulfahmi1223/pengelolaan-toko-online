...
$banyakDataPerHal = 5;
$banyakData = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_admin"));
...
if ((isset($_POST['Bcari'])) and $_POST['Pkeyword'] <> "") {
  ...
  $select = mysqli_query($conn, "SELECT * FROM tb_admin WHERE 
  admin_name LIKE '%$keyword%' OR
  email LIKE '%$keyword%' OR
  level LIKE '%$keyword%'
  LIMIT $dataAwal,$banyakDataPerHal");
} else {
  // $select = mysqli_query($conn, "SELECT * FROM tb_product LIMIT $dataAwal,$banyakDataPerHal");
  $select = mysqli_query($conn, "SELECT * FROM tb_admin LIMIT $dataAwal,$banyakDataPerHal");
}
?>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-3 text-gray-800">Data Officer</h1>
          <!-- DataTales Example -->
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <a href="tambah-officer.php" class="btn btn-primary"><i class="fa-sharp fa-solid fa-user-plus mr-1"></i>Tambah Data</a>
              <!-- Topbar Search -->
                    ...
                    ?>
                      ...
                      <?php foreach ($select as $rows) : ?>
                        <tr>
                          ...
                          <td>
                            <a href="edit-officer.php?id_admin=<?= $rows['id_admin']; ?>" class="btn btn-success text-white rounded"><i class="fa-solid fa-pen-to-square"></i></a>
                          </td>
                          <td><a href="hapus-officer.php?id_admin=<?= $rows['id_admin'] ?>" onclick="return confirm('Yakin Hapus?')" class="btn btn-danger text-white rounded"><i class="fa-solid fa-trash-can"></i></a>
                          </td>
                        </tr>
                      <?php endforeach; ?>
                    <?php else : ?>
